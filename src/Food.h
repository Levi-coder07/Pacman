...
public:
	...
	int change =  (steps + 1) * 3 - 3 * (int)(sin(timeValue * 10) );
	glUniform4f(vertexColorLocation, 1.0f, 1.f, 0.3f, 1.0f);
	glDrawElements(GL_TRIANGLES,(steps + 1) * 3 , GL_UNSIGNED_INT, (void*)(((steps + 1) * 3 - change) * sizeof(float)));

	for (int i = 0; i < (steps / 2 - 2); i++)
		glDrawElements(GL_TRIANGLES,((steps + 1) * 3) * 2, GL_UNSIGNED_INT, (void*)((((steps + 1) * 6 - change * 2) + (steps + 1) * 3 + (steps + 1) * 6 * i) * sizeof(float)));

	glDrawElements(GL_TRIANGLES, (steps + 1) * 3, GL_UNSIGNED_INT,  (void*)((((steps + 1) * 3 - change) + (steps + 1) * 3 + (steps + 1) * 6 * (steps / 2 - 2)) * sizeof(float)));
